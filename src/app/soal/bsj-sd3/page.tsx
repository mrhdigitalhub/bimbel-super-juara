...
export default function DashboardKelasPage() {
  ...
  const kelas = "bsj-sd3".toLowerCase();
  const [kode, setKode] = useState("");
  const [hasil, setHasil] = useState<Hasil[]>([]);
  const [showPantau, setShowPantau] = useState(false);

  useEffect(function () {
    // FIX FINAL: VOUCHER STRICT PER KELAS - BACA KODE ADMIN APAPUN, TAPI PER KELAS
    const kelasShort = kelas.replace('bsj-','') // sd1 dari bsj-sd1
    const keysToTry = [
      "bsj_kode_aktif_" + kelas, // bsj_kode_aktif_bsj-sd1
      "bsj_kode_aktif_" + kelasShort, // bsj_kode_aktif_sd1
      `bsj_kode_aktif_bsj-${kelasShort}`, // fallback
    ]
    let k = ""
    ...
    // JANGAN pakai bsj_kode_aktif global lagi - itu penyebab SD1 muncul di SD2-SD6
    setKode(k);
    if (!k) return;
